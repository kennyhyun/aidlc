...
```cmd
bash -c "cd <project-dir> && ./aidlc/scripts/fetch-tickets.sh REI-xxxxx"
```

Unix/Linux/macOS:
```bash
./aidlc/scripts/fetch-tickets.sh REI-xxxxx
```

This will:
- Auto-detect ticket type (epic/story/task)
- Create directory: `docs/tickets/{type}/REI-xxx_{summary}/.files/`
- Download main ticket as `.files/REI-xxx.json` (includes changelog)
- Download all subtasks and linked issues as `.files/REI-yyy.json` (includes changelog)
- Convert all JSON files to markdown (`.md` files)

### Converting JSON to Markdown

```bash
./aidlc/scripts/adf-to-text.sh docs/tickets/{type}/REI-xxx_{summary}/.files/REI-xxxx.json
```

Creates: `docs/tickets/{type}/REI-xxx_{summary}/.files/REI-xxxx.md`
...
```bash
python aidlc/scripts/jira-history-diff.py docs/tickets/{type}/REI-xxx_{summary}/.files/REI-xxxxx.json
```
